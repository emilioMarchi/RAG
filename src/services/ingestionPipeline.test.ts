...
vi.mock('../config/env.js', () => ({
  env: { GEMINI_API_KEY: 'key', LLM_API_URL: 'u', LLM_API_KEY: 'k', LLM_MODEL: 'm', R2_BUCKET_NAME: 'b', CLOUDFLARE_ACCOUNT_ID: 'i', R2_ACCESS_KEY_ID: 'ak', R2_SECRET_ACCESS_KEY: 'sk', R2_PUBLIC_DOMAIN: null }
}))
...
describe('IngestionPipeline', () => {
  ...
  it('process and store document', async () => {
    ...
    expect(res.paragraphsProcessed).toBe(2)
    expect(embedder.generateEmbedding).toHaveBeenCalled()
    expect(llm.enrichChunk).toHaveBeenCalledTimes(2)
